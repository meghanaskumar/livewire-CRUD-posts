<div class="container max-w-lg mt-10">
 @if (session()->has('message'))
    ...
    <form wire:submit.prevent="update" enctype="multipart/form-data">
           ...
           <input type="file" wire:model="image" id="image" class="w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline">
            ...
            @if ($image)
                <img src="{{ $image->temporaryUrl() }}" alt="Temporary Image"
                     class="mt-2 border border-gray-300 rounded-md shadow-sm">
        ...
        <button type="submit" class="px-4 py-2 text-white bg-blue-500 rounded hover:bg-blue-600">Update Post</button>
    </form>
</div>
</div>